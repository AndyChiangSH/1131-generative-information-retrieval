...
class BagOfWords:
    def __init__(self, corpus):
        ...
        self.doc_vectors = self._calculate_bow_vectors()

    def _calculate_bow_vectors(self):
        """ Calculate the Bag-of-Words vectors for each document. """
        bow = np.zeros((len(self.corpus), len(self.vocab)))

        # Calculate term frequency (TF) for each document
        for doc_idx, doc in enumerate(self.corpus):
            for word in doc:
                word_idx = self.vocab_index[word]
                bow[doc_idx, word_idx] += 1

        return bow

    def _calculate_query_vector(self, query):
        """ Calculate the Bag-of-Words vector for a query. """
        query_vector = np.zeros(len(self.vocab))
        query_terms = query.split()

        for term in query_terms:
            if term in self.vocab_index:
                query_vector[self.vocab_index[term]] += 1
...
if __name__ == '__main__':
    ...
    # Create Bag-of-Words instance for documents
    print("> Create Bag-of-Words instance for documents...")
    bow_model = BagOfWords(document_texts)

    # Compute similarity for train questions using Bag-of-Words
    print("> Compute similarity for train questions using Bag-of-Words...")
    train_similarity_results_bow = []
    for question in tqdm.tqdm(train_question_texts, desc="Train"):
        top_3_indices = bow_model.get_top_n(question, n=3)
        top_3_indices = [index + 1 for index in top_3_indices]
        train_similarity_results_bow.append(top_3_indices)
    ...
    submission_bow = pd.DataFrame({
        ...
        'answer': [" ".join(map(str, indices)) for indices in train_similarity_results_bow]
    })
    submission_bow.to_csv(f'answer/train/{answer_filename}.csv', index=False)

    # Calculate Recall@3 for train questions using Bag-of-Words
    print("> Calculate Recall@3 for train questions using Bag-of-Words...")
    train_hits_bow = 0
    for idx, row in train_questions.iterrows():
        true_doc_id = row['Answer ID']
        if true_doc_id in train_similarity_results_bow[idx]:
            train_hits_bow += 1

    train_recall_at_3_bow = train_hits_bow / len(train_questions)
    print(f'Recall@3 on train questions using Bag-of-Words: {train_recall_at_3_bow}')

    # Compute similarity for test questions using Bag-of-Words
    print("> Compute similarity for test questions using Bag-of-Words...")
    test_similarity_results_bow = []
    for question in tqdm.tqdm(test_question_texts, desc="Test"):
        top_3_indices = bow_model.get_top_n(question, n=3)
        top_3_indices = [index + 1 for index in top_3_indices]
        test_similarity_results_bow.append(top_3_indices)

    # Save to CSV for test answers
    print("> Save to CSV for test answers...")
    submission_bow_test = pd.DataFrame({
        'index': test_questions['Question ID'],
        'answer': [" ".join(map(str, indices)) for indices in test_similarity_results_bow]
    })
    submission_bow_test.to_csv(f'answer/test/{answer_filename}.csv', index=False)
